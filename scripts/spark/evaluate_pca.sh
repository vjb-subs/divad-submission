...
make_window_datasets_id=$(get_make_window_datasets_id "$window_size" "$window_step" "$spark_balancing" "$normal_data_prop" "$anomaly_augmentation" "$ano_augment_n_per_normal")
echo "make_window_datasets_id: ${make_window_datasets_id}"
train_window_model_id="pca"
echo "train_window_model_id: ${train_window_model_id}"
train_window_scorer_id=$(get_train_window_scorer_id "$detector_name" "$method" "$n_selected_components")
...
for scores_avg_beta in 0 0.8 0.9 0.95 0.96667 0.975 0.98 0.98333 0.9875 0.99167 0.99375 0.995; do
  ...
  detector.train_window_scorer.method="$method" \
  detector.train_window_scorer.n_selected_components="$n_selected_components" \
