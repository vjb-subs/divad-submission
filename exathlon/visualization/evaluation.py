...
from detection.metrics.helpers import get_auc


def plot_pr_curves(
    recalls, precisions, ts, *, fig_title=None, colors=None, full_output_path=None
):
    ...
    if type(recalls) != dict:
        assert (
            colors is None or type(colors) != dict
        ), "color must be passed as a single value if one curve"
        ...
        for index, text in zip(
            [0, -1], [f"Threshold = {min_ts:.2f}", f"Threshold = {max_ts:.2f}"]
        ):
        ...
        ax.plot(
            ...
            precisions,
            color=c,
            label=f"(AUC = {get_auc(recalls, precisions):.2f})",
        )
    # if `recalls` is a dict, plot one PR curve per key
    else:
        assert (
            colors is None or type(colors) == dict
        ), "colors must be passed as a dict if multiple curves"
        # only show minimum and maximum thresholds for the "mixed" curve if it exists
        if "mixed" in recalls:
            ax.scatter(
                [recalls["mixed"][0], recalls["mixed"][-1]],
                [precisions[0], precisions[-1]],
                color="r",
                s=75,
                marker=".",
            ...
            for index, text in zip(
                ...
                ax.annotate(
                    text,
                    (recalls["mixed"][index], precisions[index]),
                    ha="center",
